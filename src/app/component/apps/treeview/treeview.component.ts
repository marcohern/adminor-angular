...
import {
  ...
  LoadmoreFlatNode,
  LoadmoreNode,
} from './particularData/particularData.service';
import {
  DynamicDatabase,
  DynamicDataSource,
  DynamicFlatNode,
} from './treeViewServic/treeView.service';
...
export class TreeviewComponent {
  ...
  constructor(database: DynamicDatabase, private _database: LoadmoreDatabase) {
    ...
    this.dataSourceStatic2.data = Static_TREE_DATA;
    this.dataSourceStatic3.data = Static_TREE_DATA;
    this.dataSourceStatic4.data = Static_TREE_DATA;
    // for dynamic loading data

    this.treeControl1 = new FlatTreeControl<DynamicFlatNode>(
      this.getLevel,
      this.isExpandable
    );
    ...
    this.treeControl2 = new FlatTreeControl<LoadmoreFlatNode>(
      this.getLevel2,
      this.isExpandable2
    );
  ...
  treeControlStatic1 = new NestedTreeControl<staticTreeNode>(
    (node) => node.children
  );
  treeControlStatic2 = new NestedTreeControl<staticTreeNode>(
    (node) => node.children
  );
  treeControlStatic3 = new NestedTreeControl<staticTreeNode>(
    (node) => node.children
  );
  treeControlStatic4 = new NestedTreeControl<staticTreeNode>(
    (node) => node.children
  );

  dataSourceStatic1 = new MatTreeNestedDataSource<staticTreeNode>();
  dataSourceStatic2 = new MatTreeNestedDataSource<staticTreeNode>();
  dataSourceStatic3 = new MatTreeNestedDataSource<staticTreeNode>();
  dataSourceStatic4 = new MatTreeNestedDataSource<staticTreeNode>();

  hasChild = (_: number, node: treeNode) =>
    !!node.children && node.children.length > 0;
  hasChildStatic1 = (_: number, node: staticTreeNode) =>
    !!node.children && node.children.length > 0;
  // End of static data
  ...
  isLoadMore = (_: number, _nodeData: LoadmoreFlatNode, node: LoadmoreNode) =>
